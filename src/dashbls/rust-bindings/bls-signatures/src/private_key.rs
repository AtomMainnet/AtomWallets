...
impl PrivateKey {
    ...
    pub fn generate_atom(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "atom_helpers")]
    pub fn generate_atom_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_atom(rng))
            .collect()
    }
