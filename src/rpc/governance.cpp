...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 TOM irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on atomcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Atom features */
    { "atom",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "atom",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "atom",               "gobject",                &gobject,                {} },
    { "atom",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
