...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //atom-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Atom
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Atom";
#else
    // Unix-like
    return pathRet / ".atom";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty atom.conf if it does not exist
