...
The Atom Core daemon can be configured to act as a trusted "border
router", implementing the atom wire protocol and relay, making
...
The ZeroMQ feature in Atom Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building atomd:
...
For instance:
    ...
    $ atomd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/atomd.tx.raw \
...
These options can also be provided in atom.conf.
...
From the perspective of atomd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into atomd directly. Furthermore, no information is
...
using. Atomd appends an up-counting sequence number to each
