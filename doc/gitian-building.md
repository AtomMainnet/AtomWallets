...
*Setup instructions for a Gitian build of Atom Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Atom
...
- [Building Atom Core](#building-atom-core)
...
Clone the git repositories for Atom Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/AtomMainnet
git clone https://github.com/atompay/gitian.sigs.git
```
...
in the Atom Core repository under 'Fetch and create inputs' to install sources which require
...
To build Atom Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Atom Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/atom/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/AtomMainnet
...
For example:
```bash
URL=https://github.com/crowning-/atom.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit atom=${COMMIT} --url atom=${URL} ../atom/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit atom=${COMMIT} --url atom=${URL} ../atom/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit atom=${COMMIT} --url atom=${URL} ../atom/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the atom git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../atom/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/atom
SIGPATH=/some/root/path/atom-detached-sigs

./bin/gbuild --url atom=${BTCPATH},signature=${SIGPATH} ../atom/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/atom-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/atom-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/atom-osx-build.assert
```
...
[atom/gitian.sigs](https://github.com/atompay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
