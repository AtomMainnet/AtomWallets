...
Currently, 10% of block rewards are set aside for the Atom DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Atom in a
credit pool. This credit pool mirrors the available credit supply in Atom
Platform. When a certain amount of Atom is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Atom
Platform. Masternode identities receive the fees and can convert them back to
Atom by withdrawing them from Atom Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the AtomMainnet locked in all previous
  blocks minus all the AtomMainnet unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from Atom Core (BIP61)

This feature has been disabled by default since Atom Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Atom Core version 20.0.0 there are extra changes:
...
- It is possible to run Atom Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `TOM/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Atom Core (BIP61)_](#removal-of-reject-network-messages-from-atom-core-bip61) above for
...
- The `atom-cli -getinfo` command now displays the wallet name and balance for
...
python test framework and `atom-cli` tool both take advantage of this, so
for example:

```sh
atom-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
atom-cli -named createwallet mywallet load_on_startup=1
```
...
with Atom’s product needs, such as SegWit and RBF, are excluded from our
...
- [v19.3.0](https://github.com/AtomMainnet/blob/master/doc/release-notes/atom/release-notes-19.3.0.md) released July/31/2023
- [v19.2.0](https://github.com/AtomMainnet/blob/master/doc/release-notes/atom/release-notes-19.2.0.md) released June/19/2023
...
[DIP-0023]: https://github.com/atompay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/atompay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/AtomMainnet/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/AtomMainnet/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/AtomMainnet/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/AtomMainnet/compare/v19.3.0...atompay:v20.0.0
